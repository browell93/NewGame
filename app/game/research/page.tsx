...
import { queueEconomyResearchAction } from "@/server/actions/research";
import { getPrimaryCityDashboard } from "@/server/services/city-dashboard";
import { getCityResearchQueues } from "@/server/services/research-queues";
...
export default async function ResearchPage({ searchParams }: { searchParams?: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  if (!user) {
    redirect("/auth/sign-in?error=Please%20sign%20in%20to%20open%20research");
  }
  ...
  const queues = await getCityResearchQueues(supabase as never, dashboard.city.id);
  ...
  return (
    <section className="space-y-6">
      ...
      <DashboardPanel title="Research queues" eyebrow="Milestone 5">
        <p className="text-sm text-slate-300">Queue Economy research to validate the academy research pipeline.</p>
        <form action={queueEconomyResearchAction} className="mt-4">
      ...
      <DashboardPanel title="Active and queued research" eyebrow="Queue state">
        {queues.length === 0 ? (
          <p className="text-sm text-slate-300">No queued research yet.</p>
        ) : (
          <div className="space-y-2">
            {queues.map((q) => (
              <article key={q.id} className="rounded-xl border border-white/10 bg-slate-950/50 px-3 py-2 text-sm text-slate-200">
                {q.researchKey} → Lv. {q.targetLevel} ({q.status}) • ends {new Date(q.endsAt).toLocaleString()}
              </article>
            ))}
          </div>
        )}
