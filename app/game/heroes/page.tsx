...
import { assignFirstHeroAction } from "@/server/actions/hero-assignments";
import { recruitHeroAction } from "@/server/actions/heroes";
import { getCityHeroAssignments } from "@/server/services/hero-assignments";
...
export default async function HeroesPage({ searchParams }: { searchParams?: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const assignments = await getCityHeroAssignments(supabase as never, dashboard.city.id);
    ...
    <section className="space-y-6">
      ...
      <DashboardPanel title="Hero roster" eyebrow="Milestone 6">
        ...
        <div className="mt-4 flex flex-wrap gap-3">
          <form action={recruitHeroAction}>
            <button type="submit" className="rounded-xl border border-amber-300/30 bg-amber-300/10 px-3 py-2 text-sm font-semibold text-amber-100">Recruit hero</button>
          </form>
          <form action={assignFirstHeroAction}>
            <button type="submit" className="rounded-xl border border-emerald-300/30 bg-emerald-300/10 px-3 py-2 text-sm font-semibold text-emerald-100">Assign first hero</button>
          </form>
        </div>
      </DashboardPanel>
      ...
      <DashboardPanel title="Hero assignments" eyebrow="Milestone 17">
        {assignments.length === 0 ? (
          <p className="text-sm text-slate-300">No hero assignments yet.</p>
        ) : (
          <div className="space-y-2">
            {assignments.map((assignment) => (
              <article key={`${assignment.heroId}-${assignment.assignedAt}`} className="rounded-xl border border-emerald-300/20 bg-emerald-300/10 px-3 py-2 text-sm text-emerald-100">
                Hero {assignment.heroId} • {assignment.assignmentType} • {new Date(assignment.assignedAt).toLocaleString()}
              </article>
            ))}
          </div>
        )}
